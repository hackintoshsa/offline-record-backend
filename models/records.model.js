...
export const createRecord = async (title, description, barcode) => {
    const query = 'INSERT INTO public.records (id, title, description, barcode) VALUES ($1, $2, $3, $4) RETURNING *';
...
export const getRecords = async () => {
    const query = 'SELECT * FROM public.records';
...
export const updateRecord = async (id, title, description, barcode) => {
    const query = 'UPDATE public.records SET title = $1, description = $2, barcode = $3, updated_at = NOW() WHERE id = $4 RETURNING *';
...
export const syncRecords = async (records) => {
    const query = `
    INSERT INTO public.records (id, title, description, barcode, updated_at)
    VALUES ($1, $2, $3, $4, NOW())
    ON CONFLICT (id) 
